/**
 * Milestone sources CRUD endpoints.
 *
 *   GET  /api/milestone-sources                   → list all sources
 *   POST /api/milestone-sources { slug, name }    → create new source
 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    const row = await createMilestoneSource({ slug, name });
